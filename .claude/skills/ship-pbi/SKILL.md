---
name: ship-pbi
description: Implement a brainfog PBI end-to-end using subagents: Implementor writes code, Critic reviews, loop repeats until green, then close-pbi verifies and closes the task. Works for Claude Code and OpenCode.
---

# Skill: ship-pbi

Orchestrates the full PBI lifecycle: implement -> deterministic gates -> probabilistic review -> loop until green -> close.
...
### Step 1 - Implement (`@implementor` Subagent)
...
If gates fail and the implementor already tried to fix them, treat this as a Critic iteration failure. Otherwise give the implementor one more pass with the gate output as context before counting.

### Step 3 - Probabilistic Review (`@critic` Subagent)
...
3. Pass the Critic's full report verbatim to the next implementor pass.
...
5. Do not skip deterministic gates between implementor passes.
6. Record iteration count and each Critic report in a `## Ship-PBI Log` section appended to the PBI file when the run is substantial.

## OpenCode

OpenCode has its own version of this skill at `.opencode/skills/ship-pbi/SKILL.md` with OpenCode-native agent names (`@pbi-implementor`, `@critic`), the `apply-critic-review` skill in the fix loop, and local skill-loadable dependencies.
