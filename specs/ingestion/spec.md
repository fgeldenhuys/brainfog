...
- **API Contracts**:
  ...
  - MCP tools may be added for connector inspection and manual runs if useful for agents. Credential create/update may remain REST-only in v1 if that is safer for secret handling.
...
- [ ] A Cloudflare egress spike verifies and documents whether Garmin Connect login/API access works from Cloudflare-hosted runtime using `python-garminconnect` or an equivalent minimal probe, without committing credentials or personal payloads.
- [ ] Garmin Cloudflare-hosted runner ingestion accepts or produces bounded payloads for daily summary metrics and activities, validates them, normalizes them into `garmin.daily.*` and `garmin.activities.*` time-series points, and records run history.
- [ ] Garmin runner documentation explains that the connector uses unofficial Garmin Connect access via `python-garminconnect` in a Cloudflare-hosted runner, not Garmin's official business API, and documents known Cloudflare egress/auth risks.
- [ ] Tests cover connector CRUD, encrypted credential storage, per-user credential isolation, run lifecycle, idempotency, Garmin daily summary ingestion, Garmin activity ingestion, auth rejection, and existing time-series behavior.
  Partial evidence: PBI-017 adds credential-specific test coverage in apps/worker/test/ingestion-credentials.test.ts covering ciphertext storage, safe GET responses, credential replacement, revoke/delete, cross-user isolation, encryption key failure modes, and redacted summary generation. Garmin-specific ingestion tests remain future PBI scope.
