...
- [x] `create_task`/`update_task`/`list_tasks` (including `priority` and `recurrence`), `create_project`/`list_projects`, and `record_time_series_point`/`list_time_series_points` work over their tables, scoped to the caller's `owner_id`; `upsert_person`/`list_people` work over the global authenticated people pool.
