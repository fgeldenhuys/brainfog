...
Every non-junction memory table except `people` carries `owner_id`, `source`, and timestamps per `ARCHITECTURE.md` invariant 4 (provenance — "every memory has provenance: source agent/tool, user/token, project/scope, timestamps; writes without provenance are rejected"), except `document_chunks`, whose ownership/provenance is derived from its parent `documents` row. `people` is a global authenticated pool: all authenticated users can list, reference, and set `users.self_person_id` to the same canonical person rows. If an implementation keeps `people.owner_id` for migration compatibility or creator provenance, that column is not used for people visibility or self-person validation. `project_id` is added to `thoughts`, `tasks`, `facts`, `documents`, and `time_series_points` (the entities that meaningfully belong to a project); `people` and `projects` themselves are not project-scoped.
...
- **API Contracts**:
  ...
  MCP tools exposed under `/mcp` (extending the platform baseline's scaffold, ADR-003), all requiring a valid bearer token (ADR-004) and operating only on rows owned by the authenticated user, except that `people` rows are globally visible to all authenticated users:
  - `remember(content, type?, project_id?, links?)` → creates a `thoughts` row (`type` defaults to `observation`), generates its embedding via Workers AI, and upserts it into Vectorize using the thought row `id` as the vector ID. `links` is an optional `{ people_ids?, task_ids?, fact_ids?, document_ids? }` of existing record IDs to associate via the dependency graph; people IDs may name any global person, while task/fact/document IDs must be owned by the same user.
  ...
  - `update_fact(id, statement?, citations?, confidence?, status?, topics?, supersedes_fact_id?, superseded_by_fact_id?)` → updates a fact and its lifecycle fields, re-embedding when `statement` changes. Status changes to `superseded`/`proven_wrong` do not delete the fact or its vector; they preserve recallable history with lifecycle metadata.
    - `add_document(title, content, project_id?, mime_type?)` → writes `content` to R2 (ADR-008), creates a `documents` row with the resulting `r2_key`, splits indexable text into `document_chunks`, and generates+upserts an embedding per chunk using the chunk row `id` as the vector ID. This inline text path is text-only: `application/pdf` is rejected so PDFs must use the direct-upload path, where they are indexable only when text can be extracted or when the caller later supplies extracted text through the companion PDF-text path.
    - `update_document(id, content)` → rewrites the R2 object at the existing `r2_key`, deletes the document's existing `document_chunks` (and their Vectorize vectors), and re-chunks/re-embeds as in `add_document`. This inline text path is text-only; `application/pdf` documents must use direct upload instead.
  ...
  - `upsert_person(id?, name, aliases?, contact_info?, notes?)`, `list_people()` — CRUD over the global authenticated `people` pool. No embedding. `list_people()` returns the shared pool to any authenticated user. When `id` names an existing person, `aliases`/`contact_info`/`notes` are partial-update fields: omitting one preserves its current stored value, while an explicit value (including `[]`, `{}`, or `null`) replaces it; the existing row is found globally, not by caller ownership.
  - `whoami()` — returns the authenticated user's `id`, `name`, `self_person_id`, and resolved `self_person` (or `null` if unset). Mirrors `GET /api/v1/whoami` (`specs/platform-setup/spec.md`).
  - `create_project(name, description?)`, `list_projects()` — CRUD over `projects`.
   ...
   - `create_document_upload_link` / `update_document_upload_link` style MCP tools for document file transfer are kept intentionally opaque; bytes move through authenticated REST upload/download URLs, not inline in MCP. PDF uploads may request `indexing_mode=auto|skip`; `auto` attempts text extraction, `skip` stores the PDF without indexing.
   - `index_document_text(document_id, content)` is the companion MCP tool for manually supplying OCR/extracted text for an existing PDF document.

   REST routes under `/api/v1/*` (same auth as the platform baseline) mirror the above for use by the web UI (ADR-007): list/create routes for `projects`, `people`, `tasks`, `facts`, `thoughts`, `documents`, and `time-series-points` (with `GET /api/v1/documents/:id/content` proxying the R2 object), plus `GET /api/v1/recall?q=...` mirroring the `recall` tool. `POST /api/v1/documents/:id/indexed-text` accepts manually supplied OCR/extracted text for an existing PDF document. Request/response shapes follow the same fields as the Data Models below; `owner_id` is always derived from the authenticated token and is never accepted from the client.
...
- **Data Models** (D1, via Drizzle — ADR-002; all `id` columns are app-generated typed brainfog IDs; all timestamps are Unix seconds; `created_at`/`updated_at` exist on every non-junction table below except `document_chunks`, whose rows are regenerated rather than updated and therefore carry only `created_at`; `updated_at` is refreshed on every update where present):
  ...
  - **`people`**
    - `id` (pk), `source`, `name`, `aliases` (JSON array of strings, default `[]`), `contact_info` (JSON object, default `{}` — freeform contact details, e.g. email, phone, social handles; shape not enforced by schema), `notes` (nullable), `created_at`, `updated_at`. Implementations may retain `owner_id` (fk → `users.id`) as creator/migration metadata; it is not a visibility or validation scope.
    - Global to all authenticated users; no `project_id`, no embedding.
  ...
  - **`documents`**
    ...
    - Full content lives in R2 at `r2_key`; D1 holds metadata only. Whether a document is semantically indexed depends on whether `document_chunks` exist for it.
  ...
  - **`document_chunks`**
    ...
    - Extracted/manual OCR text uses the same chunking and embedding path as normal text documents.
  ...
  - **Thought junction tables** (composite primary key on both columns, both FKs `ON DELETE CASCADE`; no own timestamps):
    ...
    - The service layer rejects links where non-person linked rows have different `owner_id`s — SQLite FKs can't express this. Person links are allowed to any global person while the thought itself must still be owned by the caller.
...
- **Constraints**:
  ...
  - Every non-junction memory table except `people` and `document_chunks` carries `owner_id`, `source`, `created_at`, `updated_at` (invariant 4); `people` carries `source`, `created_at`, and `updated_at` and is globally visible to authenticated users, with any retained `owner_id` treated only as creator/migration metadata; `document_chunks` derives ownership/provenance from its parent `documents` row and carries `created_at`; `thoughts`, `tasks`, `facts`, `documents`, and `time_series_points` additionally carry a nullable `project_id`; `people` and `projects` do not.
...
- [x] Every non-junction memory table except `people` and `document_chunks` has `owner_id` (fk → `users.id`), `source`, `created_at`, `updated_at`; `people` is globally visible to authenticated users and has `source`, `created_at`, and `updated_at` (with retained `owner_id`, where present, treated only as creator/migration metadata); `document_chunks` derives ownership/provenance from its parent `documents` row and has `created_at`; `thoughts`, `tasks`, `facts`, `documents`, and `time_series_points` additionally have a nullable `project_id` (fk → `projects.id`); `updated_at` is refreshed on every update where present.
...
- [x] PDF uploads are text-extractable by default, fail closed when extraction is impossible unless `indexing_mode=skip` is requested, and accept manually supplied OCR/extracted text through a companion document-text indexing path.
...
- [x] `create_task`/`update_task`/`list_tasks` (including `priority` and `recurrence`), `create_project`/`list_projects`, and `record_time_series_point`/`record_time_series_points`/`list_time_series_points` work over their tables, scoped to the caller's `owner_id`; `upsert_person`/`list_people` work over the global authenticated people pool.
- [x] `record_time_series_points` (plural) accepts an array of time-series point objects (each with `series_key`, optional `value`/`unit`/`observed_at`/`project_id`/`metadata`) and inserts them atomically in a single batch, returning all inserted rows; all rows carry the caller's `owner_id` and source (e.g., `"rest:api"` for REST calls, `"mcp:tool"` for MCP calls); validation is all-or-nothing (if any point has invalid `project_id`, the entire batch is rejected with no partial insert).
- [x] `list_time_series_points` accepts an optional `series_prefix` parameter (e.g. `series_prefix="electricity"`) that filters to rows matching `LIKE 'prefix.%'` on `series_key`, using the existing `(owner_id, series_key, observed_at)` index efficiently; `series_prefix` and `series_key` are mutually exclusive (calling both returns an error); `series_prefix` must not contain SQL wildcards (`%` or `_`).
- [x] `whoami()` returns the authenticated user's `id`, `name`, `self_person_id`, and `self_person`, matching `GET /api/v1/whoami`.
- [x] `link` adds rows to the relevant junction table(s) or dependency graph for an existing caller-owned thought, rejecting task/fact/document links to rows owned by a different user while accepting any global person link.
- [x] Derivation junction writes reject links to rows owned by a different user and reject `fact_source_facts` self-links.
- [x] `/api/v1/*` list/create routes exist for `projects`, `people`, `tasks`, `facts`, `thoughts`, `documents`, and `time-series-points` (with `GET /api/v1/documents/:id/content` serving the R2 object), plus `POST /api/v1/documents/:id/indexed-text` and `GET /api/v1/recall`.
...
PBI-008 (2026-06-14) changed `people` from owner-scoped memory to a global authenticated pool: service validation, self-person lookup, thought links, dependency-graph entity handling, time-series person subjects, schema indexes, and migrations were updated while non-person memory entities remain owner-scoped. Verified with `pnpm check && pnpm typecheck && pnpm test` (98/98 passed); Vitest emitted the existing post-run close-timeout warning after tests passed.

2026-06-14: Added the `whoami` MCP tool as a thin wrapper over the existing `getSelfPerson`/whoami service logic, mirroring `GET /api/v1/whoami`. `apps/worker/src/memory.ts`, `apps/worker/src/mcp/index.ts`, `apps/worker/src/routes/api.ts`, and `apps/worker/test/memory.test.ts` updated; `pnpm check && pnpm typecheck && pnpm test` passed (99/99).

PBI-012 (2026-06-17) added bulk insert and prefix query features to time-series: `record_time_series_points` (plural) accepts an array of points and inserts them atomically, `list_time_series_points` accepts an optional `series_prefix` parameter for efficient `LIKE 'prefix.%'` range queries using the existing index, and both tools/routes carry the appropriate source provenance (`"rest:api"` for REST, `"mcp:tool"` for MCP). Fixed a blocking provenance violation where the bulk insert hardcoded `source: "mcp:tool"` instead of using the context's source. `apps/worker/src/memory.ts` and `apps/worker/test/memory.test.ts` updated and verified with `pnpm check && pnpm typecheck && pnpm test` (151/151 passed).
...
Feature: Memory model
  ...
  Scenario: Linking a thought to a person, a task, and a project
    Given a global person exists, and a task and project exist for the authenticated user
    ...
    Then the new thought is linked to the global person via thought_people or the dependency graph
