...
- The Contract is what PBIs are scoped against and what critics review against — write DoD items and scenarios precisely enough to verify mechanically (a command, a test, an observable response).
...
- Specs are living documents during planning/implementation of the feature, but once DoD items are checked off and the feature has shipped, treat the spec as describing the current system — update it if the system changes later, the same as you'd update any other documentation.
